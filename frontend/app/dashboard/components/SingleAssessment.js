...
const SingleAssessment = ({ assessmentId, status, level, date, onDelete }) => {
  ...
  return (
    <>
      <li key={assessmentId} className="flex justify-between gap-x-6 py-5">
        <div className="flex min-w-0 gap-x-4">
          <div className="min-w-0 flex-auto">
            ...
            <p className="mt-1 truncate text-xs/5 text-gray-500">Created on {formatDate(date)}</p>
          </div>
        </div>
        <div className="hidden shrink-0 sm:flex sm:flex-col sm:items-end">
          <button onClick={openAssessment} className="btn btn-sm">Open</button>
        </div>
        <div className="hidden shrink-0 sm:flex sm:flex-col sm:items-end">
      ...
      <div className="card-body">
        ...
        <button onClick={openAssessment} className="btn">Open</button>
      </div>
    </div>
    </>
  );
